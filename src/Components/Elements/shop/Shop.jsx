    ...
    <section className="py-10 justify-center">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-6 gap-6 px-4 md:px-10">
        {categories.map((category, index) => (
          ...
          >
            <Link to={category.path} className="block w-full items-center text-center">
              <div className="relative w-24 h-24 mb-4 flex items-center justify-center">
                <img
                  src={category.image}
                  alt={category.name}
                  className="w-full h-full object-cover  items-center justify-center rounded-full transition-transform duration-300 ease-in-out transform hover:scale-110"
                />
              </div>
